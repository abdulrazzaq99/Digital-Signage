...
function Detail({ t }: { t: Template }) {
  ...
  const doDelete = () => !remove.isPending && remove.mutate(t.id, { onSuccess: () => { toast.success("Template deleted"); router.replace("/layouts"); }, onError: (e) => toast.error(e) });
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_460px]">
        ...
        <div className="space-y-4">
          ...
          <div>
            <div className="flex items-center gap-2 text-xs font-semibold text-slate-900">Editable Fields <Badge tone="green">{fields.length} fields</Badge></div>
            <ul className="mt-2 space-y-1.5">{fields.map((f) => <li key={f.key} className="flex items-center justify-between rounded-lg border border-green-100 bg-green-50/40 px-3 py-2 text-xs"><span className="flex items-center gap-2"><Check className="h-3.5 w-3.5 rounded-full bg-green-500 p-0.5 text-white" /><span className="font-semibold text-slate-800">{f.label}</span><span className="font-mono text-[10px] text-slate-400">{f.key}</span></span><span className="flex items-center gap-1.5 text-[10px] text-slate-400">{f.type === "text" ? <Type className="h-3 w-3" /> : f.type === "image" ? <ImageIcon className="h-3 w-3" /> : <Palette className="h-3 w-3" />}{f.type}{f.required && <Badge tone="red">Required</Badge>}{f.max && <span>max {f.max}</span>}</span></li>)}</ul>
          </div>
          <div>
      ...
      <Modal open={del} onClose={() => !remove.isPending && setDel(false)} width="max-w-md">
        <ModalHeader title={`Delete "${t.name}"?`} subtitle={t.usedIn ? `${t.usedIn} customer instance${t.usedIn > 1 ? "s" : ""} use this template; the API refuses while they exist.` : "Customers will no longer be able to create content from it."} onClose={() => !remove.isPending && setDel(false)} />
        <ModalFooter><Button variant="secondary" onClick={() => setDel(false)} disabled={remove.isPending}>Cancel</Button><Button variant="danger" onClick={doDelete} disabled={remove.isPending}>{remove.isPending ? "Deleting…" : "Delete Template"}</Button></ModalFooter>
      </Modal>
    </div>
  );
}
