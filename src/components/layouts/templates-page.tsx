...
export function TemplatesPage({ initialTab = "fixed" }: { initialTab?: "fixed" | "zones" }) {
  ...
  const list = (templates.data?.data ?? []).filter((t) => t.name.toLowerCase().includes(q.toLowerCase()) && (cat === "All" || cat === t.category) && (orient === "All" || t.orientation === orient));
  const doDelete = () => del && remove.mutate(del.id, { onSuccess: () => { toast.success("Template deleted", del.name); setDel(null); }, onError: (e) => toast.error(e) });
  ...
  return (
    <div className="space-y-5">
      ...
      {tab === "fixed" ? (
        <>
          <div className="flex flex-wrap items-center gap-2"><SearchInput placeholder="Search templates..." className="w-64" value={q} onChange={(e) => setQ(e.target.value)} /><PillTabs options={[{ value: "All" as const, label: "All" }, { value: "LANDSCAPE" as const, label: "Landscape" }, { value: "PORTRAIT" as const, label: "Portrait" }]} value={orient} onChange={setOrient} /></div>
      ...
      <Modal open={!!del} onClose={() => setDel(null)} width="max-w-md">
        <ModalHeader title={`Delete "${del?.name}"?`} subtitle={del?.usedIn ? `${del.usedIn} customer instance${del.usedIn > 1 ? "s" : ""} use this template; the API will refuse while they exist.` : "Customers will no longer be able to create content from it."} onClose={() => setDel(null)} />
        <ModalFooter><Button variant="secondary" onClick={() => setDel(null)}>Cancel</Button><Button variant="danger" onClick={doDelete} disabled={remove.isPending}>{remove.isPending ? "Deleting…" : "Delete Template"}</Button></ModalFooter>
      </Modal>
    </div>
  );
}
