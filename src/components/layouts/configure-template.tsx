        ...
        {step === 2 ? (
          <Card className="px-5 py-5">
            ...
            <div className="mt-4 space-y-4">
              <div><div className="flex flex-wrap items-center justify-between gap-2"><Label required>Product Name</Label><span className="text-[10px] text-slate-400">{v.product.length}/40</span></div><Input value={v.product} onChange={(e) => setV({ ...v, product: e.target.value })} /><p className="mt-1 text-[10px] text-slate-400">Appears as the main headline</p></div>
              <div><div className="flex flex-wrap items-center justify-between gap-2"><Label required>Price</Label><span className="text-[10px] text-slate-400">{v.price.length}/15</span></div><Input value={v.price} onChange={(e) => setV({ ...v, price: e.target.value })} /><p className="mt-1 text-[10px] text-slate-400">Displayed prominently below the name</p></div>
              <div><div className="flex flex-wrap items-center justify-between gap-2"><Label>Tagline</Label><span className="text-[10px] text-slate-400">{v.tagline.length}/60</span></div><Input placeholder="Performance, redefined." value={v.tagline} onChange={(e) => setV({ ...v, tagline: e.target.value })} /><p className="mt-1 text-[10px] text-slate-400">Supporting line under the headline</p></div>
              ...
              <div><div className="flex flex-wrap items-center justify-between gap-2"><Label required>CTA Text</Label><span className="text-[10px] text-slate-400">{v.cta.length}/20</span></div><Input value={v.cta} onChange={(e) => setV({ ...v, cta: e.target.value })} /><p className="mt-1 text-[10px] text-slate-400">Call-to-action button label</p></div>
