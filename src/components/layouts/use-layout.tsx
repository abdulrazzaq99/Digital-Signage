    ...
    <div className="space-y-5">
      ...
      {phase === "select" && (
        <div className="max-w-md animate-fade-in">
          ...
          <ul className="mt-2 space-y-1.5">{screens.slice(0, 5).map((s) => <li key={s.id}><div role="button" tabIndex={0} onClick={() => setSel((x) => x.includes(s.id) ? x.filter((y) => y !== s.id) : [...x, s.id])} className={cn("flex w-full items-center gap-3 rounded-lg border px-3 py-2.5 text-left", sel.includes(s.id) ? "border-blue-300 bg-blue-50/50" : "border-slate-200 hover:bg-slate-50")}><Checkbox checked={sel.includes(s.id)} /><span className="flex-1 text-xs font-semibold text-slate-900">{s.name}</span><Badge tone={s.status === "Online" ? "green" : "slate"}>{s.status === "Online" ? "Online" : "Offline"}</Badge></div></li>)}</ul>
