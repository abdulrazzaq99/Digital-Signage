...
export function CanvasList() {
  ...
  return (
    <div className="space-y-5">
      <PageHeader title="Synchronized Canvas" subtitle="Combine adjacent compatible screens into one synchronized logical display." action={<div className="flex flex-wrap items-center gap-2"><CompanyFilter value={scope.companyId} onChange={scope.setCompanyId} allLabel="Select company" /><Button href={scope.withCompany("/screens/canvas/new")} disabled={!scope.companyId}><Plus className="h-4 w-4" /> Create Canvas</Button></div>} />
      <CompanyGate companyId={scope.companyId} what="its canvases">
                ...
                {data.map((c) => {
                  ...
                  return (
                    <TR key={c.id}>
                      ...
                      <TD><span className="flex items-center gap-1.5">{members.map((m, i) => <span key={m.screenId} className={`h-2.5 w-4 rounded-sm ${swatches[i % swatches.length]}`} />)}<span className="ml-1 text-xs font-semibold text-slate-700">{n}</span></span></TD>
                      <TD className="text-xs whitespace-nowrap">{n} × {label(members[0]?.orientation ?? "LANDSCAPE")}</TD>
