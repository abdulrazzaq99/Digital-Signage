  ...
  if (tab === "marketplace") {
    ...
    return (
      <OffersShell tab="marketplace">
        <div className="relative overflow-hidden rounded-2xl bg-slate-900 px-8 py-8 text-white"><div className="pointer-events-none absolute -right-10 -top-10 h-56 w-56 rounded-full bg-blue-600/30 blur-2xl" /><Badge tone="blue" className="uppercase">Marketplace</Badge><h2 className="mt-3 text-2xl font-bold">Exclusive Offers &amp; Promotions</h2><p className="mt-1 max-w-md text-sm text-slate-400">Discover the latest promotions and special offers available to you. Browse and contact providers directly.</p><div className="mt-5 flex flex-wrap gap-2"><div className="relative w-full sm:w-64"><Search className="pointer-events-none absolute left-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-slate-400" /><input placeholder="Search offers..." className="h-9 w-full rounded-lg border border-white/10 bg-white/10 pl-8 pr-3 text-xs text-white placeholder:text-slate-400 outline-none" /></div><Select className="w-40 [&>select]:h-9 [&>select]:border-white/10 [&>select]:bg-white/10 [&>select]:text-white" defaultValue="All"><option>All Categories</option></Select></div></div>
      ...
      <div className="space-y-3">
        {list.map((o) => (
          <div key={o.id} className="flex items-start gap-3 rounded-xl border border-slate-200 bg-white p-2.5 pr-3 shadow-sm sm:items-center sm:gap-4 sm:pr-4">
            <Link href={`/offers/${o.id}`} className="shrink-0"><img src={img(o.seed, 280, 200)} alt="" className="h-[60px] w-[80px] rounded-lg object-cover sm:h-[74px] sm:w-[104px]" /></Link>
