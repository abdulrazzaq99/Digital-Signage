...
function Detail({ c }: { c: Campaign }) {
  ...
  const totalPrizes = c.prizes.reduce((a, p) => a + p.quantity, 0);
  const remaining = c.prizes.reduce((a, p) => a + p.remaining, 0);
  ...
  return (
    <ScratchShell tab="campaigns" compact>
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2"><Button href={`/scratch-win/${c.id}/edit`} variant="secondary"><Pencil className="h-3.5 w-3.5" /> Edit Campaign</Button>{c.status === "ACTIVE" ? <Button variant="warning-outline" onClick={() => setDeact(true)}><Power className="h-3.5 w-3.5" /> Deactivate</Button> : <Button variant="success" onClick={doActivate} disabled={activate.isPending || c.prizes.length === 0}><Power className="h-3.5 w-3.5" /> {activate.isPending ? "Activating…" : "Activate"}</Button>}</div>
      </div>
