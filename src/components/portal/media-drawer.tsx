...
function MediaDrawerBody({ id, initialView, companyId, onClose }: { id: string; initialView: MediaDrawerView; companyId?: string | null; onClose: () => void }) {
  ...
  const doDelete = (force: boolean) => remove.mutate({ id, force }, { onSuccess: () => { toast.success("Media deleted"); onClose(); }, onError: (e) => setError(errorMessage(e)) });
  ...
  return (
    <>
      ...
      <div className="flex-1 space-y-4 overflow-y-auto px-5 py-5">
        ...
        {item.isError && <div className="rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-xs text-red-700">{errorMessage(item.error)}</div>}
        {item.data && (() => {
          const m = item.data;
            ...
            <>
              {view === "detail" && (
                <>
                  ...
                  {m.tags.length > 0 && <div className="flex flex-wrap gap-1.5">{m.tags.map((t) => <Badge key={t} tone="slate">{t}</Badge>)}</div>}
                  ...
                  <Button variant="secondary" className="w-full" onClick={() => setView("usedBy")}><Check className="h-3.5 w-3.5" /> View Used By <Badge tone="blue">{m.usedIn.length}</Badge></Button>
              ...
              {view === "delete" && (
                <>
                  <MediaPreview item={m} className="aspect-video overflow-hidden rounded-xl" />
                  {m.usedIn.length > 0 && <div className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-xs text-amber-800"><div className="flex items-center gap-1.5 font-semibold"><AlertTriangle className="h-3.5 w-3.5" /> This file is currently in use</div><p className="mt-1 leading-5">Deleting removes it from <span className="font-semibold">{m.usedIn.length} playlist{m.usedIn.length > 1 ? "s" : ""}</span>: {m.usedIn.map((p) => p.name).join(", ")}. Affected screens may show a shorter loop until the playlist is updated.</p></div>}
                  ...
                  <div className="flex gap-2"><Button variant="danger" className="flex-1" onClick={() => doDelete(m.usedIn.length > 0)} disabled={remove.isPending}><Trash2 className="h-3.5 w-3.5" /> {remove.isPending ? "Deleting…" : m.usedIn.length ? "Delete & remove from playlists" : "Delete Permanently"}</Button><Button variant="secondary" onClick={() => setView("detail")}>Cancel</Button></div>
                </>
              )}
            </>
          );
