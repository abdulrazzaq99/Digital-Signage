...
export function LayoutsPage({ tab, basePath = "/portal/layouts" }: { tab: "layouts" | "templates"; basePath?: string }) {
  ...
  const filtered = (templates.data?.data ?? []).filter((t) => (cat === "All" || t.category === cat) && (orient === "All" || t.orientation === orient) && t.name.toLowerCase().includes(q.toLowerCase()));
  ...
  return (
    <div className="space-y-5">
      ...
      {tab === "layouts" ? (
        ...
        <>
          <div className="flex flex-wrap items-center gap-3"><SearchInput placeholder="Search Templates..." className="w-56" value={q} onChange={(e) => setQ(e.target.value)} /><PillTabs options={categories.map((v) => ({ value: v, label: v }))} value={cat} onChange={setCat} /><PillTabs options={[{ value: "All" as const, label: "All" }, { value: "LANDSCAPE" as const, label: "Landscape" }, { value: "PORTRAIT" as const, label: "Portrait" }]} value={orient} onChange={setOrient} /></div>
