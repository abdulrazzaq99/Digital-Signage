...
function NameStep({ presetId, companyId, basePath, onCreated }: { presetId: string; companyId?: string | null; basePath: string; onCreated: (l: Layout) => void }) {
  ...
  const submit = () => create.mutate({ presetId, name: name.trim() }, { onSuccess: onCreated, onError: (e) => setError(errorMessage(e)) });
  return (
        ...
        {() => !preset ? <div className="text-sm text-slate-500">Unknown layout preset.</div> : (
          <div className="grid gap-5 xl:grid-cols-[400px_1fr]">
            ...
            <form onSubmit={(e) => { e.preventDefault(); submit(); }} className="space-y-4">
              ...
              <div><Label required>Layout name</Label><Input placeholder={`e.g. Lobby ${preset.name}`} value={name} onChange={(e) => setName(e.target.value)} autoFocus /></div>
              {error && <p className="rounded-md bg-red-50 px-3 py-2 text-xs text-red-700">{error}</p>}
              <Button type="submit" disabled={name.trim().length < 1 || create.isPending}>{create.isPending ? "Creating…" : "Continue to zones"}</Button>
            </form>
          </div>
        )}
      </QueryState>
...
function ZonesStep({ layout, companyId, basePath, onPublish }: { layout: Layout; companyId?: string | null; basePath: string; onPublish: () => void }) {
  ...
  const missing = layout.zones.filter((z) => !z.binding);
  const filled = Object.fromEntries(layout.zones.filter((z) => z.binding).map((z) => [z.index, z.binding!.name]));
  const pick = (index: number, c: Pick) => bind.mutate({ id: layout.id, index, bindingKind: c.kind, refId: c.id }, { onSuccess: () => setPicker(null), onError: (e) => toast.error(e, "Couldn't assign content") });
  const unbind = (index: number) => clear.mutate({ id: layout.id, index }, { onError: (e) => toast.error(e) });
    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-5 xl:grid-cols-[1fr_1fr]">
        <div>
          ...
          <NavyZoneDiagram zones={layout.zones} filled={filled} className="mt-2" />
      ...
      <AssignContentModal zone={picker === null ? null : layout.zones.find((z) => z.index === picker)?.name ?? null} companyId={companyId} pending={bind.isPending} onClose={() => setPicker(null)} onPick={(c) => picker !== null && pick(picker, c)} />
    </div>
  );
}
